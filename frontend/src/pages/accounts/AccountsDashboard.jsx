...
const AccountsDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Accounts Dashboard</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Financial overview of corporate expenditures and approved payments.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 transition-colors">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-4">
          <h3 className="text-base font-bold text-slate-900 dark:text-white">Recently Approved Vouchers</h3>
          <Link to="/accounts/vouchers" className="text-xs font-semibold text-[var(--color-primary-lt)] dark:text-sky-400 hover:underline">
            View all vouchers →
          </Link>
        </div>
        
        {(!recent || recent.length === 0) ? (
          <p className="text-slate-400 dark:text-slate-500 text-sm py-4">No approved vouchers found yet.</p>
        ) : (
          <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
            {recent.map((v) => (
              <div key={v.id} className="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2 hover:bg-slate-50/50 dark:hover:bg-slate-800/40 px-2 rounded-lg transition-colors">
                <div>
                  <Link to={`/accounts/vouchers/${v.id}`} className="font-semibold text-slate-900 dark:text-white hover:text-[var(--color-primary)] dark:hover:text-sky-400 transition-colors">
